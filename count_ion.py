...
class GetLmpData(Basic_Calcul):
        ...
        while len(indexcal) < len(sort_ob):
            ...
            for ix in range(len(sort_ob)):
                if ix not in indexcal:
                    tmp = sort_ob[ix]
            for ix in range(len(sort_ob)):
                if np.abs(self.get_vector(sort_ob[ix], tmp, box1[2])) < 3:
    ...
    def readcoor(self):
        ...
        while 1:
            ...
            aozz.sort()
            allz = self.find_ao(aozz, box1)
            remove_mid = []
            for ind1 in range(len(clay)):
                whe_mid = 0
                for ind2 in range(len(allz)):
                    if -2.5 < self.get_vector(allz[ind2], clay[ind1][2], box1[2]) < 2.5:
                        whe_mid = 1
                        break
                if whe_mid:
...
class Count(Basic_Calcul):
    ...
    def output(self):
        filewat = open('waternum', 'w')
        filedd = open('dd', 'w')
        outlist = []
        for perion in range(len(self.iontype)):
            outlist.append(open(self.iontype[perion]+'count', 'w'))
        for pertime in range(len(self.dumpdata.arr_box)):
            claybound = self.find_bound(pertime)
            print(' '.join([str(j) for j in self.caldd(pertime, claybound)]), file=filedd)
            self.left, self.right = self.find_edge(pertime)
            count_res = self.count_ion(pertime, claybound)
            for perion in range(len(count_res)):
                print(' '.join([str(i) for i in count_res[perion]]), file=outlist[perion])
            waternum = self.count_water(pertime, claybound)
            print(' '.join([str(i) for i in waternum]), file=filewat)
        for ix in outlist:
            ix.close()
        filewat.close()
        filedd.close()

    def caldd(self, time, claybound):
        resdd = []
        for i in range(1, len(claybound)):
            resdd.append(math.fabs(claybound[i][0]-claybound[i-1][0]))
        resdd.append(self.dumpdata.arr_box[time][2]-sum(resdd))
        return resdd
    ...
    def find_edge(self, time):
        obnum = 120
        tmp1 = [self.dumpdata.arr_clayo[time][ix][1] for ix in range(len(self.dumpdata.arr_clayo[time]))]
        tmp1.sort()
        tmp2 = [np.average(tmp1[:obnum]), np.average(tmp1[-obnum:])]
        return tmp2

    def count_ion(self, time, claybound):
    ...
    def count_water(self, time, claybound):
        fin_count = np.zeros([len(claybound)], int)
        for ix in range(len(self.dumpdata.arr_water[time])):
            if self.left < self.dumpdata.arr_water[time][ix][0][1] < self.right:
                    for kx in range(len(claybound)):
                        layer_dis = math.fabs(self.get_vector(claybound[kx][0], claybound[kx][1],
                                                              self.dumpdata.arr_box[time][2]))
                        updis = math.fabs(self.get_vector(self.dumpdata.arr_water[time][ix][0][2], claybound[kx][1],
                                                          self.dumpdata.arr_box[time][2]))
                        downdis = math.fabs(self.get_vector(self.dumpdata.arr_water[time][ix][0][2], claybound[kx][0],
                                                            self.dumpdata.arr_box[time][2]))
                        if updis < layer_dis and downdis < layer_dis:
                            fin_count[kx] += 1
        return fin_count
